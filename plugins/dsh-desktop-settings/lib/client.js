  ...
  const [mcpFormMsg, setMcpFormMsg] = useState("");
    ...
    const CHANGELOG = [
      ...
      },
      {
        ...
        date: "2026-09-20",
        items: [
          "桌面端版本 0.1.8.1（内核保持 0.1.5-rc.2，插件安装安全加固版）",
          "新增静态导入预检：装后在重启内核之前隔离测试插件能否加载，命中「命名导出缺失/模块缺失」即刻回滚——内核零崩溃、安装期间会话保持可用",
          "装后软验证加固：识别 Chromium 错误页 + API 健康探测 + 3 秒稳定窗口，不再把内核崩溃误判为「安装成功」",
          "自动回滚可见：启动因坏插件失败时自动回滚并弹出提示，不再静默移除插件",
          "防御插件更新故障：① 内置插件保护（不被回滚误卸载，修复更新 cordis 失败后整个包被卸载）② 设置插件目录自毁防御（修复更新时 resources/plugins 被清空、设置页分区消失）③ bundles 冲突自愈保护内置条目（不再把本应用内置插件移出加载列表）",
          "防误点加固：更新检查不再把跨预发布线版本（如 0.1.6-alpha.2）报为可更新；更新入口拦截跨线/降级目标，防止误点「全部更新」把稳定线组件换成开发线",
          "AI 诊断路径回滚补传安装前版本；本地 file: 依赖（兼容 shim）不再被误报可更新",
          "模型名修正：DeepSeek-V41-Flash → DeepSeek-V4.1-Flash（内核升级后重新应用）"
        ]
      },
      {
        version: "0.1.8",
