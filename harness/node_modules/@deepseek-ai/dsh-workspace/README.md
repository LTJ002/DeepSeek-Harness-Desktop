...
- **Archiving is reversible** — `unarchiveSession(sessionId)` removes a session from the durable archive set so it reappears in every grouping surface; an id not currently archived resolves without writing.
